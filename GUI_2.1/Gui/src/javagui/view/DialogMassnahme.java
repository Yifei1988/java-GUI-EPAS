...
public class DialogMassnahme extends Stage {
	...
	private static ArrayList<String> MaList_F1 = new ArrayList<String>();//用于Default
	...
	private static String kmt1 = new String();//用于Default
...
//	public static ObservableList<String> massVorschlag = null;
	...
	public DialogMassnahme() {// constructor：令方法名与类名一致，就可以直接在别的类调用此类此方法（还需查资料）
		...
		if(fehlerChangeCheck_old !=null){//若对话框打开前后缺陷类型不一致(且这不是对话框第一次打开)则重置之前的MaList和kmt
		...
		btnJa.setOnAction(new EventHandler<ActionEvent>() {// 赋值massVorschlag并关闭DialogMassnahme
			...
			public void handle(ActionEvent event) {
				...
				MaListClear();//在关闭对话框前初始化5个静态数组字符串MaList
				KMTsetNull();//在关闭对话框前初始化5个静态字符串kmt
				...
				}//把数组字符串massVorschlag的内容赋值给数组massVorschlagForTxt，用于打印出.txt
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// 用于将observablelist量massZuFehler1至5转化为Tree
		...
		for (int j = 0; j < F.length; j++) {
			...
			for (int i = 1; i < F[j].length; i++) {
				if (F[j][i].length() > 4) {// 若值不是空格(空格项的长度必然小于4，一般是2或者3)
					...
					treeM = makeBranch(F[j][i], treeU);// 将二维数组中每行第一列以后的元素生成为二级树杈
					if(MaList.size() != 0){
	...
	public VBox allInVBox(TreeView<String> tree, int Fn) {
		...
		switch (Fn){
		case (1): 
			ta1 = new TextArea();// 添加Kommentar文本框
		    ta1.setMinHeight(80);// 设定文本框的最小高度
		    ta1.setWrapText(true);// 这个命令可以去除文本框横向的拖动条
		    checkboxVBox.getChildren().add(ta1);
		    if(kmt1 != null){ta1.setText(kmt1);}
		    break;
		case (2): 
			ta2 = new TextArea();// 添加Kommentar文本框
		    ta2.setMinHeight(80);// 设定文本框的最小高度
		    ta2.setWrapText(true);// 这个命令可以去除文本框横向的拖动条
		    checkboxVBox.getChildren().add(ta2);
		    if(kmt2 != null){ta2.setText(kmt2);}
		    break;
		case (3): 
			ta3 = new TextArea();// 添加Kommentar文本框
		    ta3.setMinHeight(80);// 设定文本框的最小高度
		    ta3.setWrapText(true);// 这个命令可以去除文本框横向的拖动条
		    checkboxVBox.getChildren().add(ta3);
		    if(kmt3 != null){ta3.setText(kmt3);}
		    break;
		case (4): 
			ta4 = new TextArea();// 添加Kommentar文本框
		    ta4.setMinHeight(80);// 设定文本框的最小高度
		    ta4.setWrapText(true);// 这个命令可以去除文本框横向的拖动条
		    checkboxVBox.getChildren().add(ta4);
		    if(kmt4 != null){ta4.setText(kmt4);}
		    break;
		case (5): 
			ta5 = new TextArea();// 添加Kommentar文本框
		    ta5.setMinHeight(80);// 设定文本框的最小高度
		    ta5.setWrapText(true);// 这个命令可以去除文本框横向的拖动条
	...
	private void getInfoToString(TreeView<String> tree, int Fn) {
		...
		massVorschlag.add(tree.getRoot().getValue() + "Yifei");//录入树根内容
		getSelectedCB(tree.getRoot(), Fn);//将树根遍历，录入已勾选树物件
		...
		massVorschlag.add("Yifei");//为了空一行隔开各个Fehler
	}
	private void getSelectedCB(TreeItem<String> root, int Fn) {
		...
		for (TreeItem<String> treeitem : root.getChildren()) {
			if ((treeitem.getChildren()).isEmpty()) {//若此物件没有子物件(即叶子)则判断有无勾选
				String SorU;
	...
	private void MaListClear(){//重置记录了所选Massnahme项目的数组字符串MaList
		MaList_F1.clear();
		MaList_F2.clear();
		MaList_F3.clear();
		MaList_F4.clear();
	...
	private void KMTsetNull(){//重置记录了所选Kommentar内容的字符串kmt
		kmt1 = null;
		kmt2 = null;
		kmt3 = null;
		kmt4 = null;
